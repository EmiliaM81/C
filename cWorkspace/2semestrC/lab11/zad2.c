...
int main()
{
    ...
    if (fp != NULL)
    {
        fwrite(students, sizeof(students)/sizeof(*students), sizeof(*students), fp);
        fseek(fp, 0 , SEEK_SET);
        int count = sizeof(students)/sizeof(*students);
        Student_v2 tab[count];

        fread(tab, count, sizeof(*students), fp);
        ...
        printf("Bład w otwieraniu pliku\n");
        exit(1);
    }
